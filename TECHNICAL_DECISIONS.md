...
**Currency support**: The system is entirely currency-agnostic — amounts are stored and displayed as plain numbers with no currency type attached. All rule conditions based on amount thresholds implicitly assume a single currency. Supporting multiple currencies (ARS, USD, etc.) would require storing the currency alongside each amount, applying exchange rates when evaluating amount-based rules across currencies, and surfacing the currency in the UI.
